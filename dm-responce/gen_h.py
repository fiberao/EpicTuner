...
def calc_inv(ir):
    ...
    print(hh)
    print(hh.shape)
    json.dump(hh.tolist(), codecs.open(prefix + "/" + prefix + "_fit.json", 'w', encoding='utf-8'),
              separators=(',', ':'), sort_keys=True, indent=4)


resp = {"resp": ir.tolist(), "wfx": wfx.tolist(),
        "wfy": wfy.tolist()}
json.dump(resp, codecs.open(prefix + "/" + prefix + ".json", 'w', encoding='utf-8'),
          separators=(',', ':'), sort_keys=True, indent=4)
calc_inv(ir)

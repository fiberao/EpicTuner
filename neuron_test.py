...
def experiment(input_data):
    ...
    output_data= Variable( 
            -(torch.sin(input_data[:,0]-2) * torch.cos(input_data[:,1]-0.3) * (1. / (torch.abs(input_data[:,2]-0.5) + 1))),
            requires_grad=False)
...
def main():
    ...
    for epoch in range(epochs):
            ...
            d_loss =nn.L1Loss(size_average=True, reduce=True)(prob_fake,prob_real)
			#训练D
            opt_d.zero_grad()
            ...
            opt_g.step()
            if (epoch%100==1):
                print('Epoch: {}, d_loss: {}, g_loss: {}, g_good: {}'.format(epoch,  d_loss.data.cpu().numpy()[0],
                                                                        g_loss.data.cpu().numpy()[0],
                                                                        g_goodness.data.cpu().numpy()[0]
                                                                        ))
